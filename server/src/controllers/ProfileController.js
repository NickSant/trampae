...
const userDefault = [
	...
	'third_party_id',
	'image_url',
	'hash_url_to_change_pass',
	'req_change_pass_time',
]
...
export default {
	async profile(req, res) {
		...
		const requestedServices = await db(db.ref('services').as('s'))
			.select(['s.*', 'cat.title as cat_title'])
	...
	async forgotPass(req, res) {
		...
		u.get({ email: mail }, true)
			.then(user => {
				...
				cp.insert({ user_id: user.id, hash_url: urlHash, created_at: now })
					.then(a => {
